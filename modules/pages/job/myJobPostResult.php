...
<script>
function makesure() {
  if (confirm('Confirm Remove Job Post?')) {
    return true;
  }
  else {
    return false;
...
<?php
	  ...
	  //----------------1 All not Set
			  ...
			  $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' GROUP BY TIMESTAMP DESC ");
          ...
          <a href="#" onclick="window.open('?op=job&load=updateJob&jid=<?php echo $jobByTitleArray['jid']; ?>', 'Update Job', 'toolbar=yes,resizable=yes,scrollbars=yes,width=900,height=600,left=20,top=20');" title="Update Job Post">  Update Job , </a>
          
           <a href="?op=job&load=deleteProcess&jid=<?php echo $jobByTitleArray['jid']; ?>" onclick="return makesure();" >  Delete Post</a>
 ...
 if(!empty($partialJobTitle) && empty($partialJobCategory) && empty($partialJobCountry)){
		...
		$jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jtitle LIKE '%$partialJobTitle%' GROUP BY TIMESTAMP DESC ");
  ...
  //----------------3 Set Title and category only, country not set
   if(!empty($partialJobTitle) && !empty($partialJobCategory) && empty($partialJobCountry)){
			  ...
			  $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jtitle LIKE '%$partialJobTitle%' AND jcategory = '$partialJobCategory' GROUP BY TIMESTAMP DESC ");
...
if(!empty($partialJobTitle) && !empty($partialJobCategory) && !empty($partialJobCountry)){
			  ...
			  $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jtitle LIKE '%$partialJobTitle%' AND jcategory = '$partialJobCategory' AND jcountry =  '$partialJobCountry' GROUP BY TIMESTAMP DESC ");
...
if(!empty($partialJobTitle) && empty($partialJobCategory) && !empty($partialJobCountry)){
			   ...
			   $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jtitle LIKE '%$partialJobTitle%' AND jcountry = '$partialJobCountry' GROUP BY TIMESTAMP DESC ");
...
if(empty($partialJobTitle) && !empty($partialJobCategory) && !empty($partialJobCountry)){
			 ...
			 $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jcategory = '$partialJobCategory' AND jcountry =  '$partialJobCountry' GROUP BY TIMESTAMP DESC ");
  ...
  if(empty($partialJobTitle) && !empty($partialJobCategory) && empty($partialJobCountry)){
			  ...
			  $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jcategory = '$partialJobCategory' GROUP BY TIMESTAMP DESC ");
...
if(empty($partialJobTitle) && empty($partialJobCategory) && !empty($partialJobCountry)){
			  ...
			  $jobByTitle_query = mysql_query("SELECT * FROM jobpost WHERE uid='$uid' AND jcountry = '$partialJobCountry' GROUP BY TIMESTAMP DESC ");
