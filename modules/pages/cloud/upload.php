...
if ((isset ( $_POST ['action'] ) ? $_POST ['action'] : (isset ( $_GET ['action'] ) ? $_GET ['action'] : '')) == 'simple') {
	...
	// checking size of the upload
	$check_size = $_FILES ["my_field"] ["size"] / 1024;
	
	if ($check_size > 307200) {
		echo '<p>';
		echo '  <b>File not uploaded on the server</b><br />';
		echo '  Error: File you are uploading is over 300MB!';
		echo '</p>';
		die();
	}
	// -- end checking size
	...
	if ($handle->uploaded) {
		...
		if (($handle->file_src_mime == 'application/x-msdownload') || ($handle->file_src_mime == 'application/x-msdos-program') || ($handle->file_src_mime == 'application/x-msdos-windows') || ($handle->file_src_mime == 'application/x-download') || ($handle->file_src_mime == 'application/bat') || ($handle->file_src_mime == 'application/x-bat') || ($handle->file_src_mime == 'application/com') || ($handle->file_src_mime == 'application/x-com') || ($handle->file_src_mime == 'application/exe') || ($handle->file_src_mime == 'application/x-exe') || ($handle->file_src_mime == 'application/x-winexed') || ($handle->file_src_mime == 'application/x-winhlp') || ($handle->file_src_mime == 'application/x-winhelp') || ($handle->file_src_mime == 'application/x-javascript') || ($handle->file_src_mime == 'application/hta') || ($handle->file_src_mime == 'application/x-ms-shortcut') || ($handle->file_src_mime == 'application/octet-stream') || ($handle->file_src_mime == 'vms/exe')) {
			echo '<p>';
			echo '  <b>File not uploaded on the server</b><br />';
