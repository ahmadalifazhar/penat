...
//nk list down own media files uploaded on cloud files type 3gp

//close list down own media files uploaded on cloud files  type 3gp
/*
//nk list down own media files uploaded on cloud files type flv
$list_down_own_media = mysql_query("SELECT b.* FROM cloud b WHERE b.owner = '".$chkid."' AND type = 'flv'");
while($listed_own_media = mysql_fetch_array($list_down_own_media))
{
	echo $listed_own_media['location'];
	echo $listed_own_media['type']."<br>";
}
//close list down own media files uploaded on cloud files  type flv

//nk list down own media files uploaded on cloud files type mp4
$list_down_own_media = mysql_query("SELECT b.* FROM cloud b WHERE b.owner = '".$chkid."' AND type = 'mp4'");
while($listed_own_media = mysql_fetch_array($list_down_own_media))
{
	echo $listed_own_media['location'];
	echo $listed_own_media['type']."<br>";
}
//close list down own media files uploaded on cloud files  type mp4
*/
...
$zz = 0;
$list_down_own_media = mysql_query("SELECT c.* FROM cloud c WHERE c.owner = '$chkid'");
while($listed_own_media = mysql_fetch_array($list_down_own_media))
{
	if(($listed_own_media['type']=="3gp") || ($listed_own_media['type']=="flv") || ($listed_own_media['type']=="mp4"))
	{
		...
		?>  <div class="ex"><H5><?=$baru_rd;?> ...<br/><a href="/modules/pages/entertainment/test/try2.php?loc=<?php echo $listed_own_media['location']; ?>&type=<?php echo $listed_own_media['type']; ?>" target="iframe_a"><span class="fg-color-pink"><i class="icon-play-alt"></i> Play media</a></H5></div>
