...
if($ptype=="all")
{
	$rawtrades = mysql_query("SELECT t.* FROM trades t WHERE t.status=1 ORDER BY t.tdate DESC") or die(mysql_error());
}
else
{
	$rawtrades = mysql_query("SELECT t.* FROM trades t WHERE t.status=1 AND t.ttype='$ptype' ORDER BY t.tdate DESC") or die(mysql_error());
}
?>
</head>
