...
$rawtradesmarks = mysql_query("SELECT t.* FROM tradesmark t WHERE t.aid=$chkid ORDER BY t.mid DESC") or die(mysql_error());
...
<table>
    ...
    <tbody>
<?php
while($tradesmarks = mysql_fetch_array($rawtradesmarks))
{
	$tid = $tradesmarks['tid'];
	$rawtrades = mysql_query("SELECT t.* FROM trades t WHERE t.status=1 AND t.tid='$tid'") or die(mysql_error());
	$trades = mysql_fetch_array($rawtrades);
	if($trades==null)
	{ continue; }
...
<tr>
	...
	<td style="color:#333">
		<a href="productdetail2.php?product=<?php echo $trades['tid']; ?>" onclick="window.open(this.href, 'child', 'scrollbars,width=800,height=600'); return false">
